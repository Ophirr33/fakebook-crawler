...
(define (parse-response response)
  ...
  (cons code
        (cond [(= 200 code)
              ...
              [(or (= code 403) (= code 404) (= code 504)) #f]
              [else (begin
                      (println (format "Unknown code: ~a" code))
                      response)])))
...
(define csrf #f)
(define session #f)
(define username "001723069")
(define password "NEHRU6VZ")
(define visited (set))
(define flags (set))
(define todo (list (cdr (login))))  ;; Start off with the 302 redirect on the todo list

(define (main)
  (cond [(= (set-count flags) 5) flags]
        [(null? todo) flags]
        [else (let ([first (car todo)])
                (set! todo (cdr todo))
                (if (set-member? visited first) (main)
                    (begin (set! visited (set-add visited first))
                           (crawl first)
                           (main))))]))
...
(define (crawl url)
  (let* ([response (parse-response (get url))]
    ...
    (cond [(= 200 code)
           (set! todo (append (parse-links body) todo))
...
#;(let ([t1 (thread main)])
  (sleep 10)
  (build-list 10 (λ (x) (thread main)))
  (sleep 5)
  (build-list 40 (λ (x) (thread main))))
